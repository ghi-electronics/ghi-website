...
- **Visual Studio** — the **TinyCLR UI Application** project template and the UI Designer used in this tutorial are both Visual Studio-only. Using VS Code? Step 1 notes what to add by hand, and the end of Step 4 builds the screens in code instead of the Designer.
...
Not using Visual Studio? Create a plain **TinyCLR Application** project, add the packages above, and configure the display and touch driver by hand using the pages linked above — then pick up at Step 3, building the screens in code (see the end of Step 4).
...
*(The screenshots below predate this template and show the screen named `CounterPage` — your project has the same screen named `MainWindow`. The workflow is identical either way.)*
