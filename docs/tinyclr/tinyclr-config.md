...
![TinyCLR Config](https://docs.ghielectronics.com/software/tinyclr/images/tinyclr-config.png)
...
### Deployment Map
![Deployment Map](https://docs.ghielectronics.com/software/tinyclr/images/deployment-map.png)

The deployment map can be used to gauge the resources used by your application, but more importantly it allows you to make sure assemblies are stored in secure internal flash memory. See the [IP Protection](https://docs.ghielectronics.com/feature/ip-protection.html) page for more information about secure assemblies.
...
The `Extend heap` uses the external RAM for the system, which reduces the level of security. This is explained further under [External Memory](https://docs.ghielectronics.com/feature/external-memory.html).

`Persist slow clock` Sets the system clock speed to run at half speed, saving about 40% power consumption. See [Power Management](https://docs.ghielectronics.com/feature/power-management.html).

'Disable APP pin', `Debug Mode` and `Device Name` options change/disable the MOD and APP pins behavior. Those pins are explained under [Device Info](https://docs.ghielectronics.com/feature/device-info.html).
...
For added security, the debug interface can be disabled. Secure designs will disable the interface and implement [In-Field-Update](https://docs.ghielectronics.com/feature/in-field-update.html). You can also disable the debug interface from within your code. Please refer to the [IP Protection](https://docs.ghielectronics.com/feature/ip-protection.html) page for more information about disabling the debug interface.
