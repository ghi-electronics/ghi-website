...
```csharp
public GaugeStartPosition StartPosition { get; set; }
```

Which side the dial's OPENING (the gap between the low and high ends of the scale) sits on. Default `GaugeStartPosition.Bottom` (6 o'clock, the classic dial); Left = 9 o'clock, Top = 12 o'clock, Right = 3 o'clock. Rotates the whole dial together (arc, needle, ticks and labels).
