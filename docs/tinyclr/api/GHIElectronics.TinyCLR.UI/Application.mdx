...
### ShowOnScreenKeyboardAutomatically

```csharp
public bool ShowOnScreenKeyboardAutomatically { get; set; }
```

Whether tapping (or Select-ing) a TextBox automatically pops up the on-screen keyboard. Default true. Set to false when a physical keyboard is connected so the user types on it instead (tablet-style); the on-screen keyboard can still be shown on demand via `ShowOnScreenKeyboardFor`. When set back to true, tapping a TextBox pops the keyboard again.
...
### ShowOnScreenKeyboardFor(TextBox textBox)

```csharp
public void ShowOnScreenKeyboardFor(TextBox textBox)
```

Shows the on-screen keyboard for the given TextBox (its text is edited and written back when the user accepts). Called automatically when a TextBox is tapped unless `ShowOnScreenKeyboardAutomatically` is false; call it yourself for an on-demand keyboard button.
