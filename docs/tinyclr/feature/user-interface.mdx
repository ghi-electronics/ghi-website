...
NuGet package: **`GHIElectronics.TinyCLR.UI`** (the `GHIElectronics.TinyCLR.UI.Media` namespace ships inside it); add whichever **display** and **bus** packages your panel needs ([Displays → Native](displays.mdx#native-displays) or [Virtual](displays.mdx#virtual-displays)).

## Which do I need?

TinyCLR has two ways to put things on a screen:

- **User Interface (this page)** — a WPF-style framework: windows, panels, controls, events, and touch input, with screens laid out visually in the [Visual Studio UI Designer](#visual-studio-ui-designer). Pick it when you're building screens a user interacts with — settings pages, forms, menus, dashboards with buttons.
- **[Graphics](graphics.mdx)** — immediate-mode drawing on the same display backends. You draw shapes, text, and bitmaps yourself: the quickest way to put simple, custom visuals on screen when the full framework is more than you need.

**Try it:** the [Build a Touch UI tutorial](../tutorial/touch-ui.mdx) walks through a two-page touch app — designed in the UI Designer — in about 30 minutes.
...
class Program : Application {
    ...
    static void Main() {
        ...
        display.SetConfiguration(new ParallelDisplayControllerSettings { /* see Displays page */ });
        display.Enable();
...
For the display setup itself (timing parameters, backlight, SPI driver wiring), follow the patterns on the [Displays page](displays.mdx) — the UI library uses the same display backends.
...
class Program : Application {
    ...
    static void Main() {
        // ... set up st7735 controller (see Displays → Virtual displays)
