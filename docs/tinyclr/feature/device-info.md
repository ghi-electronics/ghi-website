...
As an alternate option, users can change the debug interface using [TinyCLR Config](https://docs.ghielectronics.com/software/tinyclr/tinyclr-config.html) tool.

:::note
Once the debug interfaces changed or disabled, it may not be possible to communicate with the device anymore. However, changing or disabling the interface does not affect the [bootloader](https://docs.ghielectronics.com/software/tinyclr/bootloader.html). Users can always enter the bootloader, which will be in serial or USB depending on MOD pin. From there a `Erase All` can be issues, manually or though the TinyCLR Config tool.
:::

## APP Pin

The [Special Pin](https://docs.ghielectronics.com/software/tinyclr/special-pins.html) APP's feature can be disabled using:
...
[TinyCLR Config](https://docs.ghielectronics.com/software/tinyclr/tinyclr-config.html) tool can also be used to disable APP feature.
