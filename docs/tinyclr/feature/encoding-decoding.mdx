...
TinyCLR provides many internal fast methods to handle many encoding and decoding tasks.
...
As TinyCLR is a subset of the full desktop .NET development system, not all desktop .NET features are supported. The following `ToString()` arguments are supported:
