...
## Secure Sockets
TLS is natively supported. See the [TLS page](tls.mdx) for details.

---

## .NET Sockets
TinyCLR's support of sockets is very similar to .NET socket support. Most .NET socket code should run with little modification. Socket support is provided as part of the TinyCLR core. Tiny CLR sockets work similar to .NET sockets -- most .NET examples are applicable to TinyCLR.
...
## IP Assignment
TinyCLR supports both static and dynamic IP addressing. With either method, an event is fired providing the local IP settings.
...
:::tip
Static IP addressing does not work with [PPP](ppp.mdx).
:::
