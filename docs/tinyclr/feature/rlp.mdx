---
title: Runtime Loadable Procedures (RLP)
sidebar_label: Runtime Loadable Procedures (RLP)
description: Call native ARM C/C++ code from C# on a microcontroller — the embedded equivalent of calling a native DLL from desktop .NET. Load compiled native code at runtime as an ELF blob, with no firmware rebuild. Ideal for DSP, tight compute loops, and microsecond-precision GPIO.
---

**Runtime Loadable Procedures (RLP)** let you call **native ARM C/C++ code** from your C# app — loaded onto the device **at runtime**, with no firmware rebuild. You compile your C into a small **ELF** file, get it onto the device (SD card, USB drive, or embedded resource), then look functions up by name and call them from managed code.

Think of it as the embedded equivalent of **calling a native DLL from desktop .NET** — you keep the convenience and safety of C# for your application logic, and drop into compiled native code for the parts that need raw speed.

Reach for RLP when you need that speed or low-level work the managed APIs don't cover:
...
:::info
RLP is designed for **computation and GPIO** — fast native loops plus the GPIO, timing, task, and event helpers shown below. If you instead need to read or write a specific peripheral register directly from C#, see [Marshal](marshal.mdx).
:::

## The easiest way to start: the project template
...
It's a complete, working solution — **seven ready-to-run examples** so you can see the whole feature without writing anything first:
...
1. **Write your C** in `rlp_src/template_src.c`.
2. **Double-click `rlp_src/build.bat`** to compile it into `rlp_src/app.elf`. *(Requires the Arm GNU toolchain `arm-none-eabi-gcc` 8.3.1 or newer at `C:\gcc`; edit `GCC_ROOT` in the script if yours is installed elsewhere.)*
...
:::tip
**Changing only the C/C++ code? You don't need to rebuild the C# project.** Just re-run `build.bat` to regenerate `app.elf`, then reload it onto the device — copy the new file to the SD card, drop it on the USB drive, or rebuild if it's an embedded resource. The C# app reads the fresh ELF at startup.
:::

## How it works

The ELF is just a `byte[]`. Hand the bytes to `ElfImage`, which loads the code and data into device memory and lets you find exported functions by name:
...
On the C side, every exported function has the same shape: it takes `void** args` (one slot per argument you pass from C#) and returns an `int`. Mark each export with `RLP_EXPORT` so the linker keeps it:
...
### Lookup tables and persistent state
...
`app.elf` is just bytes — the device doesn't care where they come from. The template's `LoadAppElf()` helper shows three options; pick the one that fits your hardware:

- **(A) SD card** *(default)* — copy `app.elf` to the card root, insert it, and read it with `File.ReadAllBytes`.
- **(B) USB mass storage** — plug the device into your PC; it appears as a removable drive. Drop `app.elf` on it, mount `UsbHostMassStorage` on the device, and read the file.
- **(C) Embedded resource** — add `app.elf` to the project as a binary resource and load it with `Resources.GetBytes(...)`. No removable media required; the ELF ships inside your app.

Options (A) and (B) mean **you can update the native code without rebuilding the C# app** — just replace the ELF file.
...
:::warning
**RLP is an advanced, native-code feature.** Your C runs directly on the CPU, so bugs in your logic — bad pointers, infinite loops, and the like — can hang the device. Keep RLP functions small and focused, and validate the arguments coming from C#.
:::
