...
This example shows how to communicate with Azure IoT Hub using MQTT. User will need to set up a Network Interface connection on the device such as [WiFi](wifi.mdx) to Connect to Azure.

First download the required Azure Root Certificate and add to resources. The [TLS Client](tls-client.mdx) shows how to download the root certificate.
...
![VS Output Window](/img/docs/vs_output.jpg) 