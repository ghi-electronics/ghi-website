...
:::tip
Optionally, users may utilize the built-in [cryptography](cryptography.mdx) services to encrypt the data before it is loaded into the `UnmanagedBuffer`
:::

---

## Graphical Memory
When the [graphics](graphics.mdx) engine detects available external memory, it automatically uses it. Also, garbage collection will dispose of unmanaged graphics buffers automatically. You do not have to dispose of unmanaged graphics buffers like you do for unmanaged non-graphic buffers.
