...
controller.OpenPin(SC20260.GpioPin.PH6, PinMode.Output);

while (true) {
    controller.Write(SC20260.GpioPin.PH6, PinValue.High);
    Thread.Sleep(100);

    controller.Write(SC20260.GpioPin.PH6, PinValue.Low);
    Thread.Sleep(100);
}
```
...
using GpioController controller = new GpioController();
controller.OpenPin(SC20260.GpioPin.PD7, PinMode.InputPullUp);

while (true) {
    if (controller.Read(SC20260.GpioPin.PD7) == PinValue.Low) {
        // Button is pressed.
    }
