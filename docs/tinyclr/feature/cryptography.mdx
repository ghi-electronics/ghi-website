...
TinyCLR supports multiple algorithms.  
...
XTEA encryption is not only dependent upon the supplied key, but also the "number of rounds" or iterations the encryption algorithm uses to encode and decode information. TinyCLR always uses 32 rounds. So, for example, if you are using a PC to decode data that was encoded using TinyCLR, make sure that both the correct key and number of rounds (32) are used on the PC side.
