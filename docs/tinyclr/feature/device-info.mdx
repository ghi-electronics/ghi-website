...
MOD pin is normally used to select the debug interface as explained in the [Debugging](debugging.mdx) tutorial. However, the MOD pin feature can be disabled and the debug interface can be forced to a specific interface.

```csharp
DeviceInformation.SetDebugInterface(DebugInterface.Usb);
DeviceInformation.SetDebugInterface(DebugInterface.Serial);
```
The debug interface can be completely disabled for the managed application's [IP Protection](ip-protection.mdx) `DeviceInformation.SetDebugInterface(DebugInterface.Disable);`
