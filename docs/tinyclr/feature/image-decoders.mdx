...
TinyCLR OS provides methods for converting JPG, BMP, and GIF files into bitmaps that can be used in your application. The image can be added to your project as a [resource](resources.mdx), or the image can be loaded from a byte array, like from and SD card.
...
## Video playback
Video playback can be achieved through [Motion JPEG](mjpeg-video.mdx).