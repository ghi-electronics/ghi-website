...
Fonts can be included in your TinyCLR application by adding them as a [resource](resources.md). Any TrueType font can be used after being converted to the .tcfnt format with the FontConverter tool. The Font Converter tool (found under [downloads](https://docs.ghielectronics.com/software/tinyclr/downloads.html) is a command line utility which does just that. Make sure to use this tool for any fonts you need, using old fonts from NETMF will not work.
