...
MQTT is a light weight messaging protocol for sensors that is supported by all major cloud services. See the following cloud provider pages for further details: [Azure](azure.mdx), [AWS](aws.mdx) and [Google Cloud](google-cloud.mdx).
...
:::note
The MQTT driver in TinyCLR supports client mode only.
:::
