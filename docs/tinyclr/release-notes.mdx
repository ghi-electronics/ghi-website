...
- SC20xxx: while a thread is blocked in a blocking socket `Accept`/`Receive`, `Thread.Sleep` on other threads only wakes on the lwIP slow-timer tick, quantizing wake-ups to ~250 ms (short sleeps can be delayed by up to ~250 ms). Workaround: use non-blocking sockets with `Socket.Poll` instead of a blocking `Accept`/`Receive` — polling is unaffected.
