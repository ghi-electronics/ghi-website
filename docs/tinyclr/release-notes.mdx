...
- Fixed firmware update timing out when the device was running an idle application: the device did not act on the "enter bootloader" command until it woke on its own. It now responds to the update request promptly over both USB and serial (COM) connections.

### Libraries

- `TinyCLR.UI` and all other libraries: version updated to match this release. No API or behavior changes; the UI improvements this cycle are in the UI Designer below.

### Visual Studio Extension

Fixes

- Resolved all four UI Designer known issues from 3.0.1.1000.
