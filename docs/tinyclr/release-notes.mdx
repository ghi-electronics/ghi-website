...
## 3.0.2.1000 (2026-09-08) RTW

Firmware fixes for analog inputs and the CLR, plus mass storage identification. Update the firmware, the extension, and the libraries together.

### Firmware

- **Analog in**: the internal temperature sensor, internal reference voltage, and VBAT channels now map and read correctly.
- **Analog in**: fixed a spurious exception when reading a channel while USB is enumerating.
- **CLR**: fixed an overloaded virtual method binding to the wrong overload.
- **CLR**: fixed `ToString()`, `Equals()`, and `GetHashCode()` on an unconstrained generic holding a reference type.
- **USB mass storage**: the SCSI product revision level is now settable.

### Libraries

- `GHIElectronics.TinyCLR.Devices.UsbClient`: `MassStorage.AttachLogicalUnit` accepts SCSI vendor, product, and revision strings.
- `GHIElectronics.TinyCLR.Pins`: corrected the `InternalReferenceVoltage` and `VBAT` ADC channel numbers.
- All other libraries: version updated to match this release. No API or behavior changes.
...
## 3.0.1.6000 (2026-08-27) RTW

3.0.1.6000 is an RTW release: it improves QSPI handling for large applications that run from external deployment, and adds `GroupBox` background and header colors. All other libraries and tools were rebuilt only to keep versions in sync.

### Firmware

- Improved QSPI handling that may help avoid a potential issue when performing an in-field update (IFU) of a large application that uses external deployment.

### Libraries

- `GHIElectronics.TinyCLR.UI`: `GroupBox` now supports a background color and a header color.
- All other libraries: version updated to match this release. No API or behavior changes.

### Visual Studio Extension

- **UI Designer**: `GroupBox` now supports a background color and a header color.
