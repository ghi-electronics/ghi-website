...
- **UI Designer**: in a `Grid`, a control whose `Grid.Row` / `Grid.Column` is greater than the number of defined rows/columns overlaps other controls in the preview. Add matching `RowDefinition` / `ColumnDefinition` entries to the `Grid`.
- **UI Designer**: a control inside a `ScrollViewer` cannot be selected by clicking it on the canvas. Select a control nested inside it and press **Esc** to step up to the parent.
