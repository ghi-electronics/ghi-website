...
If desired, the APP pin feature can be disabled so the application can't be stopped. See [Device Info](https://docs.ghielectronics.com/feature/device-info.html) for more details.

## MOD

The MOD pin is used to select the debugging/deployment interface, as explained in the [Debugging](https://docs.ghielectronics.com/feature/debugging.html) tutorial. An internal pull up keeps the pin high during power up to select USB debugging. Setting the pin low will switch to serial debugging.

## WKUP

The WKUP pin is used to wake the system up from Shut Down mode. See the [Power Management](https://docs.ghielectronics.com/feature/power-management.html) page for more information.

## VBAT

The VBAT pin on SITCore SoCs and SoMs is used to provide battery power to the [Real Time Clock](https://docs.ghielectronics.com/feature/real-time-clock.html) (RTC) and its battery backed memory.

## Debug Interface

USB or Serial can be used for [debugging](https://docs.ghielectronics.com/feature/debugging.html).