 ...
 * small, machine-readable `static/bin/fw/firmware.json`. The Visual Studio
...
function parseArgs(argv) {
  ...
  args.out = args.out ? path.resolve(args.out) : path.join(WEBSITE_ROOT, 'static', 'bin', 'fw', 'firmware.json');
  return args;
}
 ...
 */
const FAMILY_HEADING = /^###\s+SITCore\s+(SC\d+)\S*/gim;
const TABLE_ROW =
  /^\s*\[([^\]]+)\]\(([^)]+)\)\s*\|\s*([^|\n]+?)\s*\|\s*([^|\n]+?)\s*\|\s*([0-9A-Fa-f]+)\s*$/m;
...
function parseFamilies(section) {
  ...
  for (let i = 0; i < headings.length; i++) {
    const h = headings[i];
    const family = h[1].toUpperCase(); // "SC20"
    ...
    if (!row) {
      console.warn(`[fw] WARN: no current firmware row found under "SITCore ${family}"; skipping.`);
      continue;
    }
    ...
    families.push({
      id: family.toLowerCase(),      // "sc20"
      match: [family],               // ["SC20"] — substring-matched against the device name
...
async function main() {
  ...
  for (const f of families)
    console.log(`[fw]   ${f.match[0].padEnd(5)} display ${f.version.padEnd(12)} device ${f.deviceVersion.padEnd(10)} value ${f.value}  ${f.url}`);
}
