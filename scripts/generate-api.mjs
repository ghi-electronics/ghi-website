...
async function main() {
  ...
  // Sort by the folder name (dirOf), so the overview table is in the SAME order as
  // the autogenerated sidebar (which sorts by folder name) — e.g. the System.* compat
  // shims sort under "S", after the GHIElectronics.TinyCLR.* packages, in both places.
  const assemblies = [...byAsm.keys()].sort((a, b) => dirOf(a).localeCompare(dirOf(b)));
